...
if (droit_utilisateur("admin")) {
	...
	<body class="application">
	   <table class="menu_haut_resultat">
	      <tr>
	         <td class="menu_haut">
	          Import / Export <? echo ucfirst($table) ?>
	         </td>
	      </tr>
	   </table>
		<form method="post" action="admin_import.php?ACTE=1&TABLE=<? echo $table ?>" ENCTYPE="multipart/form-data">
		   <table class="menu_haut_resultat">
		      <tr>
		         <td class="menu_haut">
		      ...
		      foreach ($titre as $n=>$libelle) {
				...
				foreach($champs as $champ=>$proprietes) {
					if ($proprietes[clee] and !($proprietes[present])) {
						die("le champs $champ de la clée n'est pas présent dans le fichier");	
					}
				}		      
						...
						foreach ($ordre as $n=>$champ) {
							...
							if ($champs[$champ][req]) {
								if (!isset($champs[$champ][valeurs][$valeur])) {
									$erreur = "valeur $valeur pour $champ non trouvée";
								} else {
