...
if ($ACTE == 1) {
	  $message = validateIncompatibiliteLimites($_POST);
      //if(!$min)$min="NULL";
      //if(!$max)$max="NULL";
  	  if (!$message) {
  	     	   	$SR = $SR ? "1" : "0";
  	     	   ...
  	     	   if($Id_incompatibilite){	
  	     	         new db_sql("update incompatibilites_limites set 
  	     	         	produit='$produit',
  	     	         	prop1='$prop1',
  	     	         	val1='$val1',
  	     	         	prop2='$prop2',
						...
						new db_sql("insert incompatibilites_limites (produit,prop1,val1,prop2,min,max) values (
				     '$produit',
				     '$prop1',
				     '$val1',
				     '$prop2',
	...
	$req = new db_sql(" DELETE FROM incompatibilites_limites WHERE id_incompatibilite = $Id_incompatibilite ");
}
...
<FORM method=post name=formulaire action=incompatibilite_limites_detail.php?ACTE=1>
  ...
  <td class="cadre_application">
   <table class="menu_haut">
    <tr>
     ...
     <td class='externe'><A class='externe' href='incompatibilite_limites_list.php?liste_go=1'>Retour à la liste</A></td>
    </tr>
   </table> 
  </td>
 </tr>
