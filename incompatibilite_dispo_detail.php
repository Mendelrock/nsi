...
function validateIncompatibilitelist($record){
   if(!$_POST[produit])	
...
if ($ACTE == 1) {
	$message = validateIncompatibilitelist($_POST);
	if (!$message) {
		$SR = $SR ? "1" : "0";
		$PS = $PS ? "1" : "0";
		$FDV = $FDV ? "1" : "0";
  	     	   if($Id_incompatibilite){	
  	     	         new db_sql("update incompatibilites_liste set 
  	     	         	produit='$produit',
  	     	         	prop1='$prop1',
  	     	         	val1='$val1',
  	     	         	prop2='$prop2',
  	     	         	val2=".My_sql_format($val2)."
  	     	         	where id_incompatibilite=$Id_incompatibilite");
					} else {
						new db_sql("insert incompatibilites_liste (produit,prop1,val1,prop2,val2) values (
				     '$produit',
				     '$prop1',
				     '$val1',
				     '$prop2',
	...
	$req = new db_sql(" DELETE FROM incompatibilites_liste WHERE id_incompatibilite = $Id_incompatibilite ");
}
...
<FORM method=post name=formulaire action=incompatibilite_dispo_detail.php?ACTE=1>
  ...
  <td class="cadre_application">
   <table class="menu_haut">
    <tr>
     ...
     <td class='externe'><A class='externe' href='incompatibilite_dispo_list.php?liste_go=1'>Retour à la liste</A></td>
    </tr>
   </table> 
  </td>
 </tr>
