...
function showFenetreToile(ope){
   if(ope==1){	
      ...
      if(is_int(id_toile)){
         ...
         var targetWin = window.open('toile_detail.php?Id_toile='+id_toile, 'Toile', 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=530, height=330, top='+top+', left='+left);
      }
      ...
      var left = (screen.width/2)-(600/2);
      var top = (screen.height/2)-(600/2);
      var targetWin = window.open('toile_detail.php', 'Toile', 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=530, height=330, top='+top+', left='+left);	
   }
}
</script>
<?
      ...
      foreach ($liste_enregistrement as $i => $record) {
  	  ...
  	  if (!$message) {
			...
			new db_sql ("delete from article_propriete where propriete ='$propriete' and id_article=$Id_article");
  	        ...
  	        foreach ($liste_enregistrement as $i => $record) {
  	     	   $Libelle_aff = trim($_POST[$record[lb_article_aff]]);
  	     	   $Libelle = trim($_POST[$record[lb_article]]);
  	     	   $Type = $_POST[$record[type]];
  	     	   $Selecteur= $_POST[$record[selecteur]];
  	     	   $Poids = $_POST[$record[poids]];
  	     	   $Laize = $_POST[$record[laize]];
  	     	   $Qtestocklo = $_POST[Qtestocklo];
  	     	   $Qtemini = $_POST[$record[qt_mini]];
  	     	   $Qtemax = $_POST[$record[qt_max]];
  	     	   $Id_toile= $_POST[$record[id_toile]];
  	     	   $Actif= $_POST[$record[actif]];
			   $CoefChute = $_POST[$record[coef_chute]];
			   $delai = $_POST[$record[delai]];
  	     	   $IN= $_POST[$record['IN']];  $FDV= $_POST[$record['FDV']];
  	     	   $IN = $IN ? "1" : "0";
  	     	   $FDV = $FDV ? "1" : "0";
  	     	   $Id_famille = $_POST[$record[id_famille]]?$_POST[$record[id_famille]]:"null";
  	     	   ...
  	     	   if($record[typerecord]=="entete"){
					if($Id_article) {	
						$qt_max=$Qtemax;
						$qt_mini=$Qtemini;	
						$delta_stock = $Qtestocklo - get_stock_article($Id_article)[logique];
						$tran = new db_sql("update article set lb_article_aff='$Libelle_aff',lb_article='$Libelle',type='$Type',selecteur='$Selecteur',poids=$Poids,laize=$Laize,qt_stock=qt_stock + $delta_stock,qt_mini=$Qtemini,qt_max=$Qtemax,id_toile=$Id_toile, actif=".($Actif*1).", `IN`='$IN',FDV='$FDV',coef_chute = ".($CoefChute*1).",delai = ".($delai*1)."  where id_article=$i");
						unset($GLOBALS[STOCK]);
					} else {
						$tran = new db_sql("insert article (lb_article,lb_article_aff,type,selecteur,poids,laize,qt_stock,qt_mini,qt_max,id_toile, actif,`IN`,FDV, coef_chute) values ('$Libelle', '$Libelle_aff', '$Type','$Selecteur',$Poids,$Laize,$Qtestocklo,$Qtemini,$Qtemax,$Id_toile,".($Actif*1).",'$IN','$FDV', ".($CoefChute*1).", ".($delai*1).")");  
						unset($GLOBALS[STOCK]);
					...
					if($record[typerecord]=="ligneprincipal") {
						...
						$principal=0;
					if(!$id_frns_avant)
						$tran = new db_sql("insert into fournisseur_article (id_article,id_fournisseur,principal,reference,quotite,prix) values ($Id_article, $id_fournisseur,$principal,'$reference',$quotite,$prix)");  	
					else
						$tran = new db_sql("update fournisseur_article set id_fournisseur=$id_fournisseur,reference='$reference',quotite=$quotite,prix=$prix where id_fournisseur=$id_frns_avant and id_article=$Id_article");  		
				// Propriété
					...
					if(!$id_propriete_avant)
						$tran = new db_sql("insert into article_propriete (id_article,propriete,valeur) values ($Id_article,'$propriete','$valeur')");  	
					else
						$tran = new db_sql("update article_propriete set propriete='$propriete',valeur='$valeur' where propriete='$id_propriete_avant' and id_article=$Id_article");
				}
 		    }   
  	     }	
  	  }
...
if ($message) {
	...
	$resultat->n();
	$Libelle_aff=$resultat->f("lb_article_aff");
	...
	$Actif = $resultat->f("actif");
	$IN = $resultat->f("IN");
	$FDV = $resultat->f("FDV");
	$Id_famille     = $resultat->f("id_famille");
	$CoefChute = $resultat->f("coef_chute");
	$delai = $resultat->f("delai");
} else {
	$Actif = 1;	
}
...
<FORM method=post name=formulaire action="article_detail.php?ACTE=1">
...
<table class="requeteur">
   <tr>
      <td class="requet_right">Libellé affiché</td>
      <td class="requet_left" ><? champ_ouvert_droit("article","Libelle_aff",$Libelle_aff,60, 35, "O"); ?></td>
   ...
   <?
	if ($Id_article) {
		$lo_qtestock = get_stock_article($Id_article);
	} else {
		$lo_qtestock = [];
	}
   ?>
   <tr>
      <td class="requet_right">Qté stock physique</td>
      <td class="requet_left"><input type='text' disabled="true" value="<? echo $lo_qtestock[physique]; ?>" size=10></td>
      <td class="requet_right">Qté en instance</td>
      <td class="requet_left"><input type='text' disabled="true" value="<? echo $lo_qtestock[achete] ?>" size=10></td>
      <td class="requet_right">Qté stock logique</td>
      <td class="requet_left"><input type='text' <? echo $disable ?> name="Qtestocklo" value="<? echo $lo_qtestock[logique] ?>" size=10"></td>
   </tr>
   <tr>
      ...
      <td class="requet_left"><? drop_down_droit("article","", "Actif", "", "",$Actif,false, "article","O", "0|1|2", "Inactif|Actif|Fin de vie"); ?></td>
   </tr>
   <tr>
      <td class="requet_right">Libellé (lien avec les sites)</td>
      <td class="requet_left"><? champ_ouvert_droit("article","Libelle",$Libelle,60, 35, "O"); ?></td>
	  <td class="requet_right">Famille</td>
      <td class="requet_left"><? drop_down_droit("article","SELECT id_famille,lb_famille FROM familles ORDER BY lb_famille", "Id_famille", "id_famille", "lb_famille",$Id_famille,false, "article","O", "", " ","id_famille"); ?>
      <td class="requet_right">Coef Chute (%)</td>
	  <td class="requet_left"><? champ_ouvert_droit("article","CoefChute",$CoefChute,10, 10, "O"); ?></td>
   </tr>
   <tr>
		...
		<td class="requet_right">Délai de livraison (jours)</td>
		<td class="requet_left"><? champ_ouvert_droit("article","delai",$delai,10, 10, "O"); ?></td>		
    </tr>
   <?
      $liste_enregistrement[$Id_article]=array("lb_article_aff"=>"Libelle_aff","lb_article"=>"Libelle","type"=>"Type","selecteur"=>"Selecteur","poids"=>"Poids","laize"=>"Laize","qt_stock"=>"Qtestocklo","qt_mini"=>"Qtemini","qt_max"=>"Qtemax","id_toile"=>"id_toile","actif"=>"Actif","typerecord"=>"entete","IN"=>"IN","FDV"=>"FDV","id_famille" => "Id_famille", "coef_chute" => "CoefChute", "delai" => "delai"); 
   ?>
</table>
<?
if ($Id_article) {
...
<table class=requeteur>
   ...
   ?>   
      <tr>
         ...
         <td noWrap><input type='text' <? echo $disable ?> name="<? echo $Id_article.$info_principal[id_fournisseur]."_"."id_article_reference".$num_ligne ?>" value="<? echo $reference_val ?>" size=50></td>
...
if(droit_utilisateur("article")){
?>   
<INPUT class=requeteur_button name=Submit value=Enregistrer type=submit> <INPUT class=requeteur_button onclick="document.location.href='article_detail.php?Id_article=<? echo $Id_article; ?>';" name=Annuler value=Annuler type=button>
<?
}
?>
